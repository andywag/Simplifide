...
import com.simplifide.scala2.parser.nodes.IdentNode
...
case class GenericIdentType(override val name:String, override val description:String) extends GenericType(name,description) {
...
object GenericIdentType {



  trait Parser extends BaseGenericParser {
    def generic_ident(name:String,desc:String, typ:GenericType) = name ~ identDef(desc) ^^ {case nam ~ id => new Node(typ,id)}
  }

  class Node(typ:GenericType,node:IdentNode) extends GenericNode(typ) {
    override def getValue(info:BaseParserInfo):String = node.value
  }

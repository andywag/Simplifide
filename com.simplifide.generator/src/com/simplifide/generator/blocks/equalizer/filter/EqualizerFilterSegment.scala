...
class EqualizerFilterSegment {
	
}

object EqualizerFilterSegment {

  def getCommand = EqualizerFilterSegment.Commands

  val TITLE           = "parallel_equalizer_filter"
  ...
  val KEYWORDS:List[String] =  List(TITLE,"clock_head","length",FIXED_IN, FIXED_OUT, FIXED_TAP,
		  							FIXED_INTERNAL,NAME_IN,NAME_OUT,NAME_TAP)

    // PARSER SECTION
  object Parser extends BaseParser with NameLocationParser with FixedSegment.FixedParser 
  								   with GeneralSegment.Parser {
      val commandSection = EqualizerFilterSegment.Commands
      val commandString:String = EqualizerFilterSegment.TITLE

      lexical.reserved   ++= EqualizerFilterSegment.KEYWORDS
      lexical.delimiters ++= List("(",")",";",",")

      def clk    = addPosition("clock_head" ~ identDef("ClockHead")  ^^
        {case "clock_head" ~ ident   => new ClockHeadNode(ident)})

      //def length    = "length"          ~ (numberDef | identDef) ^^ {case "length" ~ num => EqualizerFilterSegment.Nodes.Length(num)}

      def widths = fixed_definition(FIXED_IN) ~
                   fixed_definition(FIXED_OUT) ~
                   fixed_definition(FIXED_TAP) ~
                   fixed_definition(FIXED_INTERNAL) ^^
        {case in ~ out ~ tap ~ inte => EqualizerFilterSegment.Nodes.Widths(in,out,tap,inte) }

      def sigs =   name_definition(NAME_IN) ~
                   name_definition(NAME_OUT) ~
                   name_definition(NAME_TAP) ^^
        {case in ~ out ~ tap  => EqualizerFilterSegment.Nodes.Signals(in,out,tap) }

      def top = clk ~ name_location ~ int_definition("length") ~ widths ~ sigs^^
        {case cl ~ na ~ le ~ widths ~ sigs => EqualizerFilterSegment.Nodes.Total(cl,na,le,widths,sigs)}
  ...
  object Nodes {
    case class Total(clk:ClockHeadNode,loc:NameLocationParser.TopNode, length:GeneralSegment.Nodes.IntDefinition,wid:Widths,sig:Signals) extends RootNode{
      val command:Command = EqualizerFilterSegment.Commands
        ...
        val signal = ParallelEqualizerFilter.Signals(clk.createFlopHead,
    	  ...
    	  return new EqualizerFilterSegment.Segment(name,loc.getName,loc.getLocation,signal,param)
      }
    }
  ...
  case class Segment(blockName:String,
                     ...
                     params:ParallelEqualizerFilter.Params) extends CommandCodeSegment(EqualizerFilterSegment.TITLE) {
    /** Template used for creating the state machine */
    override val command:Command = EqualizerFilterSegment.Commands
    ...
    object Commands extends CommandSection(EqualizerFilterSegment.TITLE) {
     ...
     override val keywords:List[String] = EqualizerFilterSegment.KEYWORDS
     /** Parser associate with this command */
     override def getParser():BaseParser = EqualizerFilterSegment.Parser
     ...
     override val command = combineCommands(EqualizerFilterSegment.TITLE,this.commands)

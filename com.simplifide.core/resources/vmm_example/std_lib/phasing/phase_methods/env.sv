...
typedef class mygroup;

class mygroup extends vmm_group;
`vmm_typename(mygroup)
...
//Methods Definations
function void mygroup::build_ph();
  vmm_phase     ph;
...
`vmm_typename(mytest)
  ...
  function new (string name, string inst, vmm_group parent=null);
    ...
    topGroup = new("tlm","topGroup", topTimeline);
  endfunction
...
initial begin
  ...
  fork 
    ...
    `vmm_note(log,"Timer expired for 'start' hence jumping to the shutdown phase ....\n");		

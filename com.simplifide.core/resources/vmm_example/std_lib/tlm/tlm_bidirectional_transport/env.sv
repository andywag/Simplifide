...
class initiator extends vmm_xactor;
  my_trans trans = new();
  vmm_tlm_nb_transport_port#(initiator, my_trans) socket = new(this, "initiator_put");
  ...
  function new(vmm_xactor parent, string name);
   ...
   virtual function vmm_tlm::sync_e nb_transport_bw(int id =-1, my_trans trans, ref vmm_tlm::phase_e p, ref int delay);
      `vmm_note(this.log, trans.psdisplay("From initiator BW path"));
   endfunction 
...
class initiator_parent extends vmm_xactor;
  ...
  vmm_tlm_nb_transport_port#(initiator_parent, my_trans) socket = new(this, "initiator_P_put");
...
class target extends vmm_xactor;
   vmm_tlm_nb_transport_export#(target, my_trans) socket = new(this, "target_put"); 
   ...
   virtual protected task main();
      ...
      forever begin
         wait(data_q.size() > 0);
         my_data = data_q.pop_front();
         `vmm_note(this.log, my_data.psdisplay("From Target BW path"));
         socket.nb_transport_bw( my_data, l_ph, l_delay);
      end
   endtask : main

   virtual function vmm_tlm::sync_e nb_transport_fw(int id =-1, my_trans trans, ref vmm_tlm::phase_e p, ref int delay);
      `vmm_note(this.log, trans.psdisplay("from target "));
      data_q.push_front(trans);
...
class target_parent extends vmm_xactor;
   target target_c;
   vmm_tlm_nb_transport_export#(target_parent, my_trans) socket = new(this, "target_P_put"); 

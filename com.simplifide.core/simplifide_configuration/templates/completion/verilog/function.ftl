...
<#macro completion_head object refer>
    ...
    <#if object.getOutputType()?exists>
    	return <@common.type>${object.getOutputType()}</@common.type>
    </#if> 
...
<#macro portlist ports>
	<#list ports as port> 
		<#if port.typeReference?exists><@common.type>${port.typeReference}</@common.type></#if> <@common.object>${port}</@common.object><#t>

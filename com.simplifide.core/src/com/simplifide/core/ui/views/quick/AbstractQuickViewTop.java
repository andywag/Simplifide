 ...
 * @since 2.1
 */
public abstract class AbstractQuickViewTop extends PopupDialog implements IInformationControl, IInformationControlExtension, IInformationControlExtension2, DisposeListener {
	 ...
	 */
	public AbstractQuickViewTop(Shell parent, int shellStyle, int treeStyle, String invokingCommandId, boolean showStatusField) {
	 ...
	 * @param treeStyle the additional styles for the tree widget
	 */
	public AbstractQuickViewTop(Shell parent, int shellStyle, int treeStyle) {
		...
		fFilterText.addKeyListener(new KeyListener() {
			public void keyPressed(KeyEvent e) {
				...
				if (e.keyCode == SWT.ARROW_DOWN)
					getfTreeViewer().getTree().setFocus();
	...
	protected void stringMatcherUpdated() {
		...
		getfTreeViewer().getControl().setRedraw(false);
		getfTreeViewer().refresh();
		getfTreeViewer().expandAll();
		selectFirstMatch();
		getfTreeViewer().getControl().setRedraw(true);
	}
	...
	protected Object getSelectedElement() {
		if (getfTreeViewer() == null)
			return null;

		return ((IStructuredSelection) getfTreeViewer().getSelection()).getFirstElement();
	}
	...
	protected void inputChanged(Object newInput, Object newSelection) {
		fFilterText.setText(""); //$NON-NLS-1$
		getfTreeViewer().setInput(newInput);
		/*
		...
		*/
		if (newSelection != null)
			getfTreeViewer().setSelection(new StructuredSelection(newSelection));
	}
	...
	public boolean hasContents() {
		return getfTreeViewer() != null && getfTreeViewer().getInput() != null;
	}
	...
	protected void setTabOrder(Composite composite) {
		if (hasHeader()) {
			composite.setTabList(new Control[] { fFilterText, getfTreeViewer().getTree() });
		} else {
			fViewMenuButtonComposite.setTabList(new Control[] { fFilterText });
			composite.setTabList(new Control[] { fViewMenuButtonComposite, getfTreeViewer().getTree() });
		}
	}

	protected void setfTreeViewer(TreeViewer fTreeViewer) {
		this.fTreeViewer = fTreeViewer;
	}

	protected TreeViewer getfTreeViewer() {
		return fTreeViewer;
	}
}
